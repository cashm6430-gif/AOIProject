#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# AOIProject - prove that a bundle works with nothing but the bundle itself
#
#   bash scripts/verify-bundle.sh [stage ...]                       # Debug
#   AOI_CACHE_CONFIG=release bash scripts/verify-bundle.sh [stage ...]
#
# Stages (default: all of them, in this order):
#   home       throwaway CONAN_HOME under out/verify/<config>/
#   restore    conan cache restore out/conan-cache-<config>.tgz into it
...
# Why this exists
# ---------------
# `package-cache.sh` can only tell you what it *put into* the archive.  That is
# not the same question as "can somebody else build with it".  Every delivery
# bug this project has hit was invisible on the producing machine:
#
...
# bundle into a throwaway home and runs the consumer's whole build from there,
# under rules that leave no room to cheat:
#
#   * the home starts empty, so nothing from ~/.conan2 can leak in
#   * the home is per configuration: a Debug restore left in the way would make
#     the "restored N binaries" check pass for an archive that shipped almost
#     nothing, and would answer a Release resolve with Debug binaries
...
#
# The developer cache is never touched.  Logs land in
# out/6-verify[-<config>].log and out/verify/<config>/*.log.
# ---------------------------------------------------------------------------
...
ARCHIVE="$ROOT/out/conan-cache-$CONFIG.tgz"
ARCHIVE_UNIX="$ROOT_UNIX/out/conan-cache-$CONFIG.tgz"
VERIFY_ROOT="$ROOT_UNIX/out/verify/$CONFIG"
...
if [ "$CONFIG" = "debug" ]; then
  ...
  LOG="$ROOT_UNIX/out/6-verify-$CONFIG.log"
fi
...
#
# Both trees this script clears are large (a restored home is hundreds of
# thousands of files) and a cache unpacked by tar can carry read-only entries,
# so drop the attribute and retry.  The errors stay visible on purpose:
# swallowing them costs a diagnostic round, and "could not clear" alone does not
# say whether it was a permission, a lock or a path-length problem.
clear_tree() {
  local dir="$1" out
  [ -e "$dir" ] || return 0
...
# Set by build_tree_is_current() so the caller can say *why* it is clearing.
BUILD_STALE_REASON=""

# True when the existing build tree can be reused by this run.
#
# Two things tie a CMake tree to where and how it was configured, and both are
# recorded in files CMake does not refresh on its own:
#
#   CMakeCache.txt                 CMAKE_CACHEFILE_DIR is the directory CMake
#                                  demands to find the tree in.  Move the tree
#                                  and it refuses to configure it at all.
#   CMakeFiles/<ver>/CMakeSystem.cmake
#                                  the absolute path of the toolchain file is
#                                  baked in and only rewritten when the compiler
#                                  changes, so a tree configured by an older
#                                  layout keeps including a file that is not
#                                  there any more.
#
# Both produce long errors that point at the toolchain rather than at the tree,
# which is why they are worth detecting up front.
build_tree_is_current() {
  local cache_file="$V_BUILD/CMakeCache.txt"
  BUILD_STALE_REASON=""
  [ -f "$cache_file" ] || { BUILD_STALE_REASON="no CMakeCache.txt"; return 1; }

  local cached_dir want_dir
  cached_dir="$(sed -n 's/^CMAKE_CACHEFILE_DIR:INTERNAL=//p' "$cache_file" \
                  | head -1 | tr -d '\r' | tr 'A-Z' 'a-z')"
  want_dir="$(printf '%s' "$(AOI_WINPATH "$V_BUILD")" | tr 'A-Z' 'a-z')"
  if [ -z "$cached_dir" ]; then
    BUILD_STALE_REASON="CMakeCache.txt has no CMAKE_CACHEFILE_DIR"
    return 1
  fi
  if [ "$cached_dir" != "$want_dir" ]; then
    BUILD_STALE_REASON="configured in $cached_dir, now at $want_dir"
    return 1
  fi

  local sys_cmake old_tc new_tc
  sys_cmake="$(find "$V_BUILD/CMakeFiles" -maxdepth 2 -name CMakeSystem.cmake 2>/dev/null | head -1)"
  [ -n "$sys_cmake" ] || { BUILD_STALE_REASON="no CMakeSystem.cmake"; return 1; }
  old_tc="$(grep -oiE '[A-Za-z]:[/\\][^"[:space:]]*conan_toolchain\.cmake' "$sys_cmake" \
              | head -1 | tr '\\' '/')"
  new_tc="$(AOI_WINPATH "$V_GEN")/build/$BUILD_TYPE/generators/conan_toolchain.cmake"
  if [ -z "$old_tc" ]; then
    BUILD_STALE_REASON="tree was not configured with a conan toolchain"
    return 1
  fi
  if [ "$(printf '%s' "$old_tc" | tr 'A-Z' 'a-z')" != \
       "$(printf '%s' "$new_tc" | tr 'A-Z' 'a-z')" ]; then
    BUILD_STALE_REASON="configured against $old_tc, this run uses $new_tc"
    return 1
  fi
  return 0
}
...
stage_home() {
  say "home: create an empty CONAN_HOME for $CONFIG"
...
stage_restore() {
  say "restore: conan cache restore ($CONFIG)"
...
stage_install() {
  say "install: conan install (--no-remote, no --build=missing, $BUILD_TYPE)"
  local -a opt_flag=()
  ...
  # --output-folder therefore leaves two includes that both define a preset,
  # and afterwards every plain
  #     cmake --preset debug
  ...
  [ -f "$V_GEN/build/$BUILD_TYPE/generators/conan_toolchain.cmake" ] \
    || { bad "no conan_toolchain.cmake under $V_GEN/build/$BUILD_TYPE"; return 1; }
  ok "conan_toolchain.cmake generated"
}

stage_build() {
  say "build: cmake configure + build (fresh tree, restored toolchain, $BUILD_TYPE)"
  require_toolchain
  prefer_bundled_ninja
  ...
  # Reusing the tree only works while it is where it was configured, and while it
  # was configured against the toolchain this run just generated.  Otherwise
  # start clean: CMake's complaints ("CMAKE_CACHEFILE_DIR ... is different than
  # the directory where CMakeCache.txt was created", "include could not find
  # requested file ... conan_toolchain.cmake") are long and read like toolchain
  # problems, while the real cause is a tree left behind by a different layout.
  if [ -e "$V_BUILD/CMakeCache.txt" ] && ! build_tree_is_current; then
    printf '      stale build tree (%s) -- clearing it\n' "$BUILD_STALE_REASON"
    clear_tree "$V_BUILD" || return 1
    mkdir -p "$V_BUILD/bin" || return 1
  fi
  ...
  "$AOI_CMAKE" -S "$ROOT" -B "$(AOI_WINPATH "$V_BUILD")" -G Ninja \
      ...
      -DCMAKE_TOOLCHAIN_FILE="$(AOI_WINPATH "$V_GEN")/build/$BUILD_TYPE/generators/conan_toolchain.cmake" \
...
stage_test() {
  say "test: ctest ($BUILD_TYPE)"
  require_toolchain
  prefer_bundled_ninja
  "$AOI_CTEST" --test-dir "$(AOI_WINPATH "$V_BUILD")" \
      --build-config "$BUILD_TYPE" --output-on-failure > "$VERIFY_ROOT/ctest.log" 2>&1
  local rc=$?
...
stage_run() {
  say "run: Shrimp.exe --selftest (out of the restored $CONFIG bundle)"
...

{
  ...
  if [ "$has_install" -eq 1 ] && [ "$has_test" -eq 1 ] && [ "$has_run" -eq 1 ]; then
    printf 'VERIFIED: the bundle alone builds, tests and runs the %s project\n' "$CONFIG"
  elif [ "$has_install" -eq 1 ] && [ "$has_test" -eq 1 ]; then
    printf 'VERIFIED: the bundle alone builds and tests the %s project\n' "$CONFIG"
  else
  ...
  printf '  stages : %s\n' "$*"
  printf '  home   : %s\n' "$V_HOME"
  printf '  logs   : out/verify/%s/*.log and %s\n' "$CONFIG" "${LOG#"$ROOT_UNIX"/out/}"
  printf '%s\n' "-------------------------------------------------------------"
} | tee -a "$LOG"
exit 0
