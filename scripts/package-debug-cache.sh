...
#
#   bash scripts/package-debug-cache.sh
#
# The corporate JFrog CE instance is not reachable from this machine, so the
# Debug binaries are built locally and shipped as a cache snapshot instead.
# On a machine that can reach the remote:
#
#   conan cache restore conan-cache-debug.tgz
#   conan upload --list=pkglist.json -r <remote> -c
#
# Artefacts written into out/:
#   conan/debug/graph-debug.json   dependency graph of the Debug build
#   conan/debug/pkglist-debug.json exact recipe+package revisions to ship
#   conan-cache-debug.tgz          the cache snapshot (the file to carry over)
# ---------------------------------------------------------------------------
...
OUT="$ROOT/out"
CONAN_OUT="$OUT/conan/debug"
GRAPH_JSON="$CONAN_OUT/graph-debug.json"
PKG_JSON="$CONAN_OUT/pkglist-debug.json"
...
say "1/5  resolve the Debug dependency graph"
...
# Everything the Debug build actually links.  Deliberately not the whole graph:
...
if [ -n "$missing" ]; then
  ...
  printf '       Nothing was written.  Two causes, in order of likelihood:\n' >&2
  printf '         1. the graph was resolved without the option the deps stage\n' >&2
  printf '            used (-o "&:with_shrimp=True"), so the conditional package\n' >&2
  printf '            is not in the graph at all;\n' >&2
  printf '         2. "-c:a tools.graph:skip_binaries=False" was missing: conan then\n' >&2
